...
const makeEffectWorkflowStoragePg = Effect.gen(function* () {
  ...
  return EffectWorkflowStorage.of({
    ...
    createExecution: input =>
      insertIgnoringDuplicate(
        sql`INSERT INTO effect_workflow_executions (execution_id, workflow_name, payload)
            VALUES (${input.executionId}, ${input.workflowName}, ${JSON.stringify(input.payload)}::jsonb)`,
      ),
    ...
    setActivityExit: (activityId, exit) =>
      sql`INSERT INTO effect_workflow_activities (activity_id, exit_value)
          VALUES (${activityId}, ${JSON.stringify(exit)}::jsonb)
          ON CONFLICT (activity_id) DO UPDATE SET exit_value = COALESCE(effect_workflow_activities.exit_value, EXCLUDED.exit_value)`.pipe(
        Effect.asVoid,
        Effect.orDie,
      ),
    ...
    setDeferred: (executionId, deferredName, exit) =>
      insertIgnoringDuplicate(
        sql`INSERT INTO effect_workflow_deferreds (execution_id, deferred_name, exit_value)
            VALUES (${executionId}, ${deferredName}, ${JSON.stringify(exit)}::jsonb)`,
      ),
...
export const EffectWorkflowStoragePgLive: Layer.Layer<EffectWorkflowStorage, never, SqlClient.SqlClient> =
  Layer.effect(EffectWorkflowStorage)(makeEffectWorkflowStoragePg);
